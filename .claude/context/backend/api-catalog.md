...
| GET | `/reviews/questions/due?limit=10` | — | `{items:[{id,reps,type,stem,options}], totalDue}` | caller-only, due wrong-answer cards backed by lesson exercises of type `mcq`/`cloze`/`reorder`; `limit` is 1–10; never returns answer/explanation |
| POST | `/reviews/{cardId}/check` | `{answer}` | `{correct, correctAnswer, explanationVi}` | read-only answer check; 400 `answer_required`; 404 for missing/unowned/unsupported source; 409 `review_not_due` |
| POST | `/reviews/{cardId}/grade` | `{rating: 1-4, operationId, expectedReps, answer?}` | 200 `{id, operationId, due, state, stability, difficulty, reps, xp, coins}` | Question-backed cards require `answer`; wrong accepts rating 1 only, correct accepts 2–4, otherwise 400 `rating_does_not_match_answer`. Legacy/manual cards retain rating-only compatibility. Same operation+payload replays; changed answer/rating/card/reps returns 409 `idempotency_conflict`. First success updates FSRS, mastery for question cards, `LearningEvent`, and reward (5 XP, 1 coin) transactionally. |
