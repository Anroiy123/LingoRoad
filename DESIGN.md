...
- Golden comparisons run only on the pinned Linux Flutter 3.44.6 CI runtime. The suites declare foundation, learning, and Progress/Profile/Settings/Streak surfaces at 390×844 in light and dark mode. Windows runs exercise layout and accessibility assertions but never create committed raster baselines.
- The five Task 3 Figma states with `golden: null` are not silently exempt: `pendingGoldenBaselines` records their route, state, exact `progress-profile/` filenames, and light/dark modes. The Linux baseline bootstrap must commit and link those twelve approved rasters before a visual-regression pass can be claimed.
- `progress_profile_golden_test.dart` uses deterministic in-memory repositories for CEFR, mastery, quests, profile initials, preferences, and a fixed injected streak month/calendar. It additionally verifies Progress, Profile, Notification Settings, Learning Goals & Schedule, and Streak have no overflow at 320, 390, and 600 widths and at 1.3 text scale, with 48px learner controls.
